...
\textcolor{red}{TODO change the traceback to a list of sets. It's too
obvious an optimisation and we have to have it for the tree algorithm
to make sense}
...
Set $i \leftarrow \argmax(V)$, $V \leftarrow V / V_i$,
...
Set $V'_{j} \leftarrow p_{t} p_{e}$ and $j \leftarrow j + 1$.
If $j < n$ go to step M3.
}

\algstep{M6.}{Site loop.}{
Set $V \leftarrow V'$ and $\ell \leftarrow \ell + 1$.
If $\ell < m$ go to step M2.
}

\algstep{M7.}{Traceback.}{
Set $P\leftarrow \npZeros(m)$, $\ell \leftarrow m - 1$ and $P_\ell \leftarrow
\argmax(V)$. Then, while $l > 0$ set $P_{l - 1} \leftarrow T_{P_\ell, \ell}$
