...
# Downloaded from https://github.com/jason2506/PythonHMM
# Modified by JK to change transition/emission probabilities to function of the
# sites.
...
class Model(object):
    ...
    def trans_prob(self, state_from, state_to, site):
        """
        ...
        if state_from not in self._states or state_to not in self._states:
            return 0
        return self._trans_prob(state_from, state_to, site)

    def emit_prob(self, state, symbol, site):
        """
        ...
        if state not in self._states or symbol not in self._symbols:
            return 0
        return self._emit_prob(state, symbol, site)
    ...
    def _forward(self, sequence):
        ...
        for state in self._states:
            alpha[0][state] = self.start_prob(state) * self.emit_prob(state, sequence[0], 0)
            ...
            for state_to in self._states:
                ...
                for state_from in self._states:
                    prob += alpha[index - 1][state_from] * \
                        self.trans_prob(state_from, state_to, index)
                alpha[index][state_to] = prob * self.emit_prob(state_to, sequence[index], index)
            ...
            for state_from in self._states:
                ...
                for state_to in self._states:
                    prob += beta[1][state_to] * \
                        ...
                        self.emit_prob(state_to, sequence[index], index)
    ...
    def decode(self, sequence):
        ...
        delta = {}
        for state in self._states:
            delta[state] = self.start_prob(state) * self.emit_prob(state, sequence[0], 0)
        ...
        for index in range(1, sequence_length):
            ...
            for state_to in self._states:
                ...
                max_state = None
                for state_from in self._states:
                    prob = delta[state_from] * self.trans_prob(state_from, state_to, index)
                    if prob > max_prob:
                        max_prob = prob
                        max_state = state_from
                delta_bar[state_to] = max_prob * self.emit_prob(state_to, sequence[index], index)
            ...
            for state_from in self._states:
                ...
                for state_to in self._states:
                    prob = alpha[index][state_from] * beta[index + 1][state_to] * \
                        self.trans_prob(state_from, state_to, index) * \
                        self.emit_prob(state_to, sequence[index + 1], index)
